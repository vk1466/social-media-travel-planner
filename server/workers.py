...
def ingest_one_link(event: dict[str, Any], context: Any = None) -> dict[str, Any]:
  ...
  try:
    ...
    result = ingest_link(
      ...
      user_id=user_id,
      refresh=refresh,
      mark_visited=mark_visited_flag,
    )
    jobs.update_item(job_id, result)
    logger.info(
      "worker ingest_one_link done job_id=%s outcome=%s post_id=%s",
      job_id,
      result.outcome,
      result.post_id,
    )
    ...
    jobs.update_item(
      job_id,
      IngestResult(post_url=post_url, outcome="error", reason=str(exc)),
    )
    return {
      "job_id": job_id,
      "post_url": post_url,
...
def _timeline_stats(result) -> dict[str, int]:
  return {
    "imported": result.imported,
    "queued_for_review": result.queued_for_review,
    "skipped_home": result.skipped_home,
    "skipped_local": result.skipped_local,
    "skipped_chain": result.skipped_chain,
    "skipped_routine": result.skipped_routine,
    "skipped_semantic": result.skipped_semantic,
    "skipped_errand": result.skipped_errand,
    "skipped_highway": result.skipped_highway,
    "skipped_address": result.skipped_address,
    "skipped_parking": result.skipped_parking,
...
def process_timeline_batch(event: dict[str, Any], context: Any = None) -> dict[str, Any]:
  ...
  user_id = event["user_id"]
  s3_key = event["s3_key"]
  item_ref = event.get("post_url") or f"timeline-batch:{event.get('batch_index', 0)}"
  ...
  try:
    jobs.mark_fetching(job_id, item_ref)
    all_clusters = clusters_from_dicts(timeline_staging.load_clusters(s3_key))
    clusters = all_clusters[batch_start : batch_start + batch_count]
...
def finalize_timeline_job(event: dict[str, Any], context: Any = None) -> dict[str, Any]:
  """Run hierarchy linking and mark a Timeline import job done."""
  del context
  job_id = event["job_id"]
  ...
  try:
    ...
    logger.exception("worker finalize_timeline_job hierarchy failed job_id=%s", job_id)
  jobs.mark_done(job_id)

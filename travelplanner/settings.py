...
def place_facts_enabled() -> bool:
  """Master switch for type-specific place facts enrichment (default off).

  Prefer ``features.enabled(PLACE_FACTS)`` at new call sites.
  """
  from travelplanner.features import PLACE_FACTS, enabled

  return enabled(PLACE_FACTS)
...
def nps_api_key() -> str | None:
  value = os.getenv("NPS_API_KEY", "").strip()
  return value or None

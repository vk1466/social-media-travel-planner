"""Wikivoyage REST / MediaWiki helpers for place-facts and travel recommendations."""
...
_SUMMARY_URL = "https://en.wikivoyage.org/api/rest_v1/page/summary/{title}"
_API_URL = "https://en.wikivoyage.org/w/api.php"


@dataclass(frozen=True)
class WikivoyageSummary:
  title: str
  extract: str
  url: str
...
def _get_json(url: str, *, timeout: float = 15.0) -> dict[str, Any] | None:
  ...
  try:
    ...
    logger.warning("wikivoyage request failed url=%s error=%s", url, exc)
    return None
...
def fetch_summary(title: str) -> WikivoyageSummary | None:
  """Fetch a short Wikivoyage travel guide summary by title. Fail-soft → None."""
  cleaned = title.strip()
  if not cleaned:
    return None
  ...
  if not url:
    url = f"https://en.wikivoyage.org/wiki/{urllib.parse.quote(page_title.replace(' ', '_'))}"
  lat, lon = _coords_from_summary(payload)
  description = payload.get("description")
  return WikivoyageSummary(
    title=page_title,
    extract=extract,
    url=url,
    latitude=lat,
...
def geosearch(
  ...
  *,
  ...
  """Nearby Wikivoyage page titles. Fail-soft → []."""
  params = urllib.parse.urlencode(
    {
      "action": "query",
      "list": "geosearch",
      "gscoord": f"{latitude}|{longitude}",
      "gsradius": max(10, min(int(radius_m), 20_000)),
...
def search_titles(query: str, *, limit: int = 5) -> list[str]:
  """Search Wikivoyage articles by keyword or place name. Fail-soft → []."""
  cleaned = query.strip()
  if not cleaned:
    return []
  params = urllib.parse.urlencode(
